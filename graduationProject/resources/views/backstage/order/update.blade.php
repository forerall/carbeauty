...
@section('content')
    <div class="page-content">
        <div class="page-header">
            <h1>
                {{$mode=='create'?'添加':''}}订单{{$mode=='edit'?'编辑':''}}{{$mode=='show'?'详情':''}}
                <small>
        ...
        <div class="row">
            <div class="col-xs-12">
                ...
                @endif
                <form class="form-horizontal" id="" action="/backstage/order{{$mode=='edit'?'/'.$item->id:''}}" method="post">
                    {{csrf_field()}}
                    @if($mode=='edit')
                        {{method_field('PUT')}}
...
<div class="form-group">
    ...
    <div class="col-xs-12 col-sm-11">
        <select name="state" class="col-xs-6 col-sm-2">
            @forelse(App\Models\Ser\Order::$stateArray as $k=>$v)
