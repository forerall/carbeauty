...
<a href="/backstage/notice/create" class="btn btn-sm">新增</a>
</div>
            <div class="col-xs-12">
...
<td><div>{{$item->begin}}</div></td>
<td><div>{{$item->end}}</div></td>
<td><div>{{$item->created_at}}</div></td>
<td width="120">
<div class="action-buttons">
<a class="blue showBtn" href="/backstage/notice/{{$item->id}}"><i class="icon-zoom-in bigger-130"></i></a>
<a class="green editBtn" href="/backstage/notice/{{$item->id}}/edit"><i class="icon-pencil bigger-130"></i></a>
<a class="red delBtn" href="javascript:void(0)"><i class="icon-trash bigger-130"></i></a>
</div>
</td></tr>
                        @empty
                            <tr>
                                <td colspan="7" class="no-record">没有记录！</td>
                            </tr>
                        @endforelse
                        </tbody>
                    </table>
...
@section('script')
    <script>
        ...
        $(function () {
            $(".delBtn").click(function(){var id = $(this).closest("tr").data("item-id");ajaxConfirm("{{url('/backstage/notice')}}/"+id,{},"DELETE",function(data){location.reload();},"确认删除");});
        })
    </script>
@stop