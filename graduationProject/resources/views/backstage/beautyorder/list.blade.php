...
@section('content')
    ...
    <div class="page-content">
        ...
        <div class="row">
            ...
            <div class="col-xs-12">
                <div class="order-nav">
                    <div{!! request('state')===null?' class="current"':'' !!}><a href="/backstage/beautyorder">全部</a></div>
                    @foreach(\App\Models\Beautyorder::$stateArray as $key=>$type)
                    ...
                    <table class="table table-striped table-bordered table-hover list">
                        ...
                        @forelse($list as $item)
                            <tr data-item-id="{{$item->id}}">
                                ...
                                <td>
                                    <div>{{$item->user->name?$item->user->name:''}}</div>
                                </td>
                                <td>
                                    <div>{{$item->project->name?$item->project->name:''}}</div>
                                </td>
                                ...
                                <td width="180">
                                    <div class="  action-buttons">
                                        <a class="blue showBtn" href="/backstage/beautyorder/{{$item->id}}"><i
                                        ...
                                        @if($item->state==1)
                                            <a class="green checkBtn" _state="2" href="javascript:void(0)"/>完成</a>
                                            <a class="red checkBtn" _state="3" href="javascript:void(0)"/>拒绝</a>
    ...
    <script>
        ...
        $(function () {
            $(".delBtn").click(function () {
                ...
                ajaxConfirm("{{url('/backstage/beautyorder')}}/" + id, {}, "DELETE", function (data) {
                    location.reload();
                }, "确认删除");
            });
            ...
            $(".checkBtn").click(function () {
                ...
                ajaxConfirm("{{url('/backstage/checkOrder')}}", {id:id,state:state}, "POST", function (data) {
                    location.reload();
                }, "确认订单");
            });
