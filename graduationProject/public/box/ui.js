...
function multiUploadBannerBox(dom, url, value, height, jump_types) {
    ...
    if (value) {
        for (i in value) {
            var t = '<div class="img-item-container"><div class="img-item-close">x</div><div class="img-item"><img style="max-height: ' + height + 'px" src="' + value[i].image + '">'
    ...
    upload_obj.fileupload({
        ...
        var t = '<div class="img-item-container">' +
            ...
            '<div class="img-item">' +
            '<img style="max-height: ' + height + 'px" src="' + data.files[0].thumb[0].webPath + '">' +
            '<input type="hidden" name="' + name + '[' + uid + '][image]" value="' + data.files[0].webPath + '">' +
            '</div>' +
            '<div class="options">' +
        ...
        t += '</select></p>' +
            ...
            '</div>' +
            '</div>';
        $(t).insertBefore(dom.find('.upload-btn'));
        upload_progress.hide();

    });

    dom.find('.upload_preview_' + name).on('click', '.img-item-close', function () {
...
function multiUploadImageBox(dom, url, value, height) {
    ...
    upload_obj.fileupload({
        ...
        for (var i in data.files) {
            $('<div class="img-item"><div class="img-item-close">x</div><img style="max-height: ' + height + 'px" src="' + data.files[i].thumb[0].webPath + '">'
                + '<input type="hidden" name="' + name + '[]" value="' + data.files[i].webPath + '"></div>').insertBefore(dom.find('.upload-btn'));
        }
        upload_progress.hide();
...
function uploadImageBox(dom, url, value, height) {
    ...
    upload_obj.fileupload({
        ...
        upload_preview.find('img').attr('src', data.files[0].thumb[0].webPath).show(200).css('max-height', height);
...
function uploadImageWithCrop(dom, url, value, size, width, height) {
    ...
    dom.append(
        ...
        + '</div>'
        + '<div>'
        + '<div class="upload-preview upload_preview_' + name + '"></div>'
        + '<div class="upload-progress upload_progress_' + name + '"></div>'
        + '</div>');

    var input = dom.find('input[name="' + name + '"]');
    var upload_obj = $('#upload_obj_' + name);
    var upload_btn = dom.find('#upload_btn_' + name);
    var upload_preview = dom.find('.upload_preview_' + name);
    var upload_progress = dom.find('.upload_progress_' + name);
    //上传事件触发
    ...
    if (value) {
        ...
        input.val(value);
        initJcrop($(dom.find('.upload_preview_' + name).find('img')[0]), name, size);
    }
    ...
    upload_obj.fileupload({
        ...
        upload_preview.find('img').attr('src', data.files[0].thumb[0].webPath).show(200).css('max-width', width).css('max-height', height);
        input.val(data.files[0].webPath);
        upload_progress.hide();
        //
        if (jcrop_api != null) {
        ...
        initJcrop($(dom.find('.upload_preview_' + name).find('img')[0]), name, false);
    });
}
...
function uploadFileBox(dom, url, value, height) {
    ...
    upload_obj.fileupload({
        ...
        var result = data.result;
        data = result.data;
        upload_preview.html(data.files[0].webPath);
        input.val(data.files[0].webPath);
        upload_progress.hide();
    });
}