...
class Image
{
    public static function centerString($dst_image, $dst_x, $dst_y, $dst_w, $dst_h, $fontSize, $text, $font = '', $color = [0, 0, 0], $charset = 'utf-8')
    {

        empty($font) && $font = __DIR__ . DIRECTORY_SEPARATOR . 'weiruanyahei.ttf';
        $rr = imagettfbbox($fontSize, 0, $font, $text);
        $textWidth = abs($rr[4] - $rr[0]);
        $textHeight = abs($rr[5] - $rr[1]);

        $x = $dst_x + ($dst_w - $textWidth) / 2;
        $y = $dst_y + $dst_h - ($dst_h - $textHeight) / 2;


        $c = imagecolorallocate($dst_image->handle, $color[0], $color[1], $color[2]);
        return imagefttext($dst_image->handle, $fontSize, 0, $x, $y, $c, $font, mb_convert_encoding($text, 'html-entities', $charset));
    }
    ...
    //横图横切，纵图纵切
    public static function thumb($src_image, $dst_w, $dst_h, $enlarge = false)
    {
        $is_w = $src_image->width >= $src_image->height;
        if ($is_w) {
            //是否允许图片放大
            if ($src_image->width < $dst_w && !$enlarge) {
                $dst_h = $src_image->height;
                $dst_w = $src_image->width;
            } else {
                $dst_h = $dst_w * $src_image->height / $src_image->width;
            }
        } else {
            //是否允许图片放大
            if ($src_image->height < $dst_h && !$enlarge) {
                $dst_h = $src_image->height;
                $dst_w = $src_image->width;
            } else {
    ...
    //按宽度切
    public static function thumbByWidth($src_image, $dst_w, $dst_h, $enlarge = false)
    {
        $dst_h = $dst_w / $src_image->width * $src_image->height;
        return self::crop($src_image, 0, 0, $dst_w, $dst_h, $src_image->width, $src_image->height, $enlarge);
    }
    ...
    //创建图像对象
    public static function imageCreate($src_image, $dst_w, $dst_h, $transparent = false)
    {
        if ($dst_w <= 0 || $dst_h <= 0) {
            return false;
        }
        ...
        if (!$transparent) {
            ...
            $color = imagecolorallocatealpha($dst, 0, 0, 0, 127);//拾取一个完全透明的颜色
            imagealphablending($dst, false);//关闭混合模式，以便透明颜色能覆盖原画板
            imagefill($dst, 0, 0, $color);
        }
        imagesavealpha($dst, true);//设置保存PNG时保留透明通道信息
