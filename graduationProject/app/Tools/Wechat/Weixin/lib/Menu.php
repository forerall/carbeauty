...
//    }
    ...
    //菜单组
    public static function btnGroup($name){
        $btn = new static('');
        $btn->name = $name;
        return $btn;
    }
    //view：跳转URL
    public static function btnView($name,$url){
        $btn = new static('view');
        $btn->name = $name;
        $btn->url = $url;
        return $btn;
    ...
    //click：点击推事件
    public static function btnClick($name,$key){
     ...
     * @return array
     */
    public static function menuObjectToArray($menu){
        $newButton = [];
        //对象转成数组
