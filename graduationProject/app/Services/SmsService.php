...
class SmsService
{
    ...
    //找回密码
    public function sendForgetPayPasswordCode($to)
    {
        return $this->sendSms($to, 'forgetPayPassword');
    }
    ...
    public function checkForgetPayPasswordCode($phone, $code, $disable = true)
    {
        return $this->checkCode($phone, $code, 'forgetPayPassword', $disable);
    }
    ...
    protected function sendSms($to, $type)
    {
        ...
        try {
            ...
            if (!$result->hasError()) {
                $sms = new Sms();
                return $sms->send($to, $result->getData()['data'], $type);
            }
            return $result;
        } catch (\Exception $e) {
            Log::error($e);
